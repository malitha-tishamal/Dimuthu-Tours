...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    if(isset($_POST['seed_defaults'])) {
        ...
        $defaults = [
            ...
            'https://images.unsplash.com/photo-1579450392631-7bc9d91f2142?q=80&w=1920', // Kandy
            'https://images.unsplash.com/photo-1552423814-147ccca0b171?q=80&w=1920', // Train
            'https://images.unsplash.com/photo-1534127055230-0ec60cf32607?q=80&w=1920', // Elephant
            'https://images.unsplash.com/photo-1564630322980-0a370e2862e6?q=80&w=1920', // Tea
            'https://images.unsplash.com/photo-1543731068-7e0f5beff43a?q=80&w=1920'  // Coastal
        ];
        foreach($defaults as $url) {
            $pdo->prepare("INSERT INTO hero_slides (image_path) VALUES (?)")->execute([$url]);
        }
        $msg = "Default 10 images seeded successfully.";
    ...
    } elseif(isset($_POST['replace_slide'])) {
        $id = $_POST['slide_id'];
        $old_path = $_POST['old_path'];
        
        if(isset($_FILES['slide_image']) && $_FILES['slide_image']['error'] == 0) {
            $ext = pathinfo($_FILES['slide_image']['name'], PATHINFO_EXTENSION);
            $filename = 'hero_repl_' . uniqid() . '.' . $ext;
            $upload_path = '../assets/hero/' . $filename;
            
            if(move_uploaded_file($_FILES['slide_image']['tmp_name'], $upload_path)) {
                // Delete old local file if it exists
                if(strpos($old_path, 'assets/hero/') === 0 && file_exists('../' . $old_path)) {
                    unlink('../' . $old_path);
                }
                ...
                $msg = "Slide replaced successfully.";
            } else {
            ...
            $stmt = $pdo->prepare("UPDATE hero_slides SET image_path = ? WHERE id = ?");
            $stmt->execute([$_POST['image_url'], $id]);
            $msg = "Slide replaced successfully.";
        }
...
$slides = $pdo->query("SELECT * FROM hero_slides ORDER BY id ASC")->fetchAll();
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <h2 class="fw-bold mb-0">Hero Slider Management</h2>
    <div class="d-flex gap-2">
        <form method="POST" onsubmit="return confirm('Replace all current slides with 10 top Sri Lankan photos?');">
             <button type="submit" name="seed_defaults" class="btn btn-outline-success"><i class="fas fa-magic me-2"></i>Seed 10 Top Photos</button>
        </form>
...
<div class="row g-4">
    ...
    <div class="col-md-4 col-lg-3">
        <div class="card h-100 shadow-sm border-0 rounded-4 overflow-hidden position-relative group">
            ...
            <div class="card-body p-2">
                <div class="d-flex gap-2">
                    <button class="btn btn-outline-info btn-sm flex-grow-1 rounded-pill" data-bs-toggle="modal" data-bs-target="#replaceSlideModal<?php echo $slide['id']; ?>"><i class="fas fa-sync-alt me-1"></i> Replace</button>
                    <form method="POST" onsubmit="return confirm('Remove this slide?');" class="flex-grow-1">
                        <input type="hidden" name="slide_id" value="<?php echo $slide['id']; ?>">
                        <button type="submit" name="delete_slide" class="btn btn-outline-danger btn-sm w-100 rounded-pill"><i class="fas fa-trash me-1"></i> Delete</button>
                    </form>
                </div>
            </div>
        </div>
    ...
    <!-- Replace Modal -->
    <div class="modal fade" id="replaceSlideModal<?php echo $slide['id']; ?>" tabindex="-1">
      <div class="modal-dialog">
        <form class="modal-content" method="POST" enctype="multipart/form-data">
          <div class="modal-header">
            <h5 class="modal-title">Replace Slide #<?php echo $slide['id']; ?></h5>
            <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
          </div>
          <div class="modal-body text-start">
            <input type="hidden" name="slide_id" value="<?php echo $slide['id']; ?>">
            <input type="hidden" name="old_path" value="<?php echo htmlspecialchars($slide['image_path']); ?>">
            <div class="mb-3">
          ...
          <div class="modal-footer">
            ...
            <button type="submit" name="replace_slide" class="btn btn-primary">Update Slide</button>
          </div>
        </form>
      </div>
    </div>
...
<style>
.group:hover { transform: translateY(-3px); transition: transform 0.3s ease; }
</style>
